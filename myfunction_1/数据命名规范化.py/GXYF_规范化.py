...
def process_all_groups(base_input_dir, base_output_dir, base_progress_file, start_group=3, end_group=8,
    ...
    for group_num in range(start_group, end_group + 1):
        ...
        # 构建具体的路径
        input_dir = os.path.join(base_input_dir, f"组{group_num}")
        output_dir = os.path.join(base_output_dir, f"组{group_num}_规范化")
...
if __name__ == '__main__':
    # 基础路径配置
    base_dir = "/media/dell/T7 Shield/nnunet/AllData/GXYF/分组数据"
    base_progress_file = "/media/dell/T7 Shield/nnunet/AllData/GXYF/KMTH_HCC_CT_38_CT_dicom_classification_HCC.json"

    # 选择处理方式：

    # 方式1: 批量处理组3到组8
    print("批量处理组3到组8")
    process_all_groups(base_dir, base_dir, base_progress_file, start_group=3, end_group=8, group_rest_duration=60)
